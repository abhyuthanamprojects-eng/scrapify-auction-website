...
import { ORDERS, inr, cr } from "@/lib/enterprise";

export const Route = createFileRoute("/portal/orders")({
  head: () => ({
    meta: [
      { title: "My Fulfilment Orders & Contracts — Scrapify Vendor Portal" },
      { name: "description", content: "Vendor order tracking, gate passes, weighbridge slips, and milestone completion." },
    ],
  }),
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <Kpi label="Active Fulfilment Orders" value={String(vendorOrders.length)} hint="Under active delivery" />
        <Kpi label="Total Contracted Value" value={cr(vendorOrders.reduce((s, o) => s + o.totalValue, 0))} hint="Signed awards" />
        <Kpi label="Gate Passes Ready" value="2 Valid" delta="QR Ready" hint="Plot 48 Plant Yard" />
      </div>

      <Card title="Active Contracts & Deliveries" desc="Real-time milestone progress and gate pass tokens">
        <Table head={["Order #", "Type", "Event Title", "Value", "Milestone Progress", "Status", "Actions"]}>
          {vendorOrders.map((o) => {
            ...
            return (
              <tr key={o.id}>
                ...
                <td className="py-3 font-semibold text-foreground max-w-xs truncate">{o.eventTitle}</td>
                <td className="py-3 font-bold font-mono">{inr(o.totalValue)}</td>
                <td className="py-3 w-36">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">{done}/{o.milestones.length}</span>
                ...
                <td className="py-3">
                  <Pill tone={o.status === "in_progress" ? "warn" : "good"}>{o.status.replace("_", " ").toUpperCase()}</Pill>
                </td>
                <td className="py-3">
                  <button
